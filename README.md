...
## Desenho da arquitetura
    ...
    │ ✨ TASK 3                │                 │                            │
    │ raw_to_trusted.sql       │─────────────────┼────────────────────────────┘
    ├──────────────────────────┤ read from Raw   │ write to Trusted
    │ • MERGE INTO Trusted     │                 │ create/write ───┐
    │ • Select key columns     │                 │                 │
    │ • Deduplicate records    │                 │                 │
...
**Task 3: Raw to Trusted** (depende de Task 2)
```
...
### Anomalia nos Dados Identificada

Durante o desenvolvimento, foi identificada uma anomalia nos dados fonte: para uma mesma combinação de `VendorID`, `tpep_pickup_datetime` e `tpep_dropoff_datetime`, existem registros com valores completamente diferentes em outras colunas (ex: `total_amount`, `passenger_count`).
...
-- Retorna múltiplos registros com valores diferentes
```

**Decisão**: Por conta desta anomalia, optou-se por **não implementar** a lógica `WHEN MATCHED THEN UPDATE SET *` no MERGE da camada Trusted, mantendo apenas INSERT de novos registros e DELETE de registros não encontrados na fonte.
